                ...
                if not mr.is_exists(
                    ...
                    if task.get("update_subdir", False) and re.search(
                        ...
                        if task.get("update_subdir_resave_mode", False):
                            ...
                            subdir_tree, _  = self.dir_check_and_save(
                                task,
                                pwd_id,
                                stoken,
                                share_file["fid"],
...
class QuarkOperator:
    ...
    def __init__(self, config_path=None):
        global CONFIG_DATA
        config_path = config_path if config_path else "quark_config.json"
        ...
        if not cookies:
            print("❌ cookie 未配置")
            return
        self.accounts = [Quark(cookie, index) for index, cookie in enumerate(cookies)]
    ...
    def do_save(self,account, tasklist=[]):
        # if not account.is_active:
        #     print(f"账号 {account.nickname} 已失效，跳过")
        #     return
        print(f"🧩 载入插件")
        is_new_tree = False
        ...
        # 获取全部保存目录fid
        ff_path = account.update_savepath_fid(tasklist)
        print(f"ff_path is {ff_path}")
        def is_time(task):
            return (
        ...
        for index, task in enumerate(tasklist):
            ...
            if not is_time(task):
                ...
                is_new_tree, tt_flid = account.do_save_task(task)
        ...
        print()
        print(f"is_new_tree: {is_new_tree}")
        # 方法2：遍历整棵树查找特定条件的节点
        for node in is_new_tree.all_nodes_itr():
            # 可以根据data中的条件过滤节点
            print(f"节点ID: {node.identifier}")
            print(f"节点数据: {node.data}")
            # print(f"parent节点的值:{node.parent}")
            print("---")
        if is_new_tree:
            return {"target_save_path":ff_path, "target_save_flid":tt_flid}
        return False


    def validate_account_and_sign(self, account):
        # 验证账号
        print(f"▶️ 验证第{account}个账号")
        account.verify_account()
        # 签到
        account.sign()

    def delete_time_file(self,fileid):
        # 删除到达时间的文件
        self.accounts[0].delete([fileid])

    # 分享链接
    def share_file(self, filidlist, title):
        task_id = self.accounts[0].share_file_for_taskid(filidlist, title)
        share_id_res = self.accounts[0].query_task(task_id)
        share_id = share_id_res["data"]["share_id"]
        res = self.accounts[0].share_file(share_id)
        print(f"分享的结果： {res}")
        return res
